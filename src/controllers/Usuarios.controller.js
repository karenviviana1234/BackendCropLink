...
export const registrarUsuarios = async (req, res) => {
  ...
  try {
    ...
    //Ayuda a verificar si hay error en el servidor 
  } catch (error) {
    res.status(500).json({
      status: 500,
...
export const actualizarUsuario = async (req, res) => {
  // validacion que viene desde el archivo validate
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    }
//parametro por el cual se va a buscar el usuario
    const { identificacion } = req.params;
    //constante que se piden para actualizar
    const { nombre, apellido, correo, password, rol } = req.body;
//condicional para que todos los campos esten diligenciados
...
//constante de usuario anterior antes de actualizar
...
export const desactivarUsuario = async (req, res) => {
  try {
    //como parametro se pide la identificacion
    const { identificacion } = req.params;
...
// constante para que se pueda actualizar los datos del usuario sin tener que volver a registrarlo
    const [oldUsuario] = await pool.query('SELECT * FROM usuarios WHERE identificacion = ?', [identificacion]);
    //condicional para ver si se cumplio el proceso, y para veirificar el usuario
    ...
    //Estos son los campos que va a tener como anteriores
    const updatedUsuario = {
      nombre: nombre || oldUsuario[0].nombre,
      apellido: apellido || oldUsuario[0].apellido,
      correo: correo || oldUsuario[0].correo,
...
export const buscarUsuario = async (req, res) => {
  try {
    //como parametro pide la identificacion
    const { identificacion } = req.params;
